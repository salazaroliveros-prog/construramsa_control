...
function auditDataIntegrity(db) {
  ...
  test('CSV export structure: diario/semanal/mensual', () => {
    ...
    movs.forEach((m, i) => {
      assert(
        typeof m.fecha === 'string' && m.fecha.match(/^\d{4}-\d{2}-\d{2}$/),
        `Movement ${i} invalid fecha: ${m.fecha}`
      );
      assert(['ingreso', 'egreso'].includes(m.tipo), `Movement ${i} invalid tipo: ${m.tipo}`);
      assert(
        typeof m.descripcion === 'string' && m.descripcion.length > 0,
        `Movement ${i} missing descripcion`
      );
      assert(typeof m.monto === 'number' && m.monto > 0, `Movement ${i} invalid monto: ${m.monto}`);
      assert(
  ...
  test('CSV export structure: asistencia', () => {
    ...
    workers.forEach((t, i) => {
      assert(
        typeof t.nombre === 'string' && t.nombre.length >= 3,
        `Worker ${i} nombre too short: ${t.nombre}`
      );
      assert(typeof t.puesto === 'string', `Worker ${i} missing puesto`);
      assert(
        typeof t.pago_hora_normal === 'number' && t.pago_hora_normal > 0,
  ...
  test('CSV export structure: mantenimiento', () => {
    ...
    insumos.forEach((ins, i) => {
      ...
      assert(
        typeof ins.articulo === 'string' && ins.articulo.length >= 2,
        `Supply ${i} articulo too short`
      );
      assert(typeof ins.cantidad === 'number' && ins.cantidad >= 1, `Supply ${i} invalid cantidad`);
      assert(typeof ins.costo === 'number' && ins.costo > 0, `Supply ${i} invalid costo`);
    });
  ...
  test('Linked expenses integrity (gasto_id references)', () => {
    ...
    datos.viajes_camiones.viajes.forEach((v) => {
      if (v.gasto_id) {
        assert(cajaIds.has(v.gasto_id), `Viaje gasto_id ${v.gasto_id} not in caja_chica`);
        linkedCount++;
      }
    });
    ...
    datos.mantenimiento.ordenes.forEach((o) => {
      if (o.gasto_id) {
        assert(cajaIds.has(o.gasto_id), `Orden gasto_id ${o.gasto_id} not in caja_chica`);
        linkedCount++;
      }
    });
    ...
    datos.mantenimiento.compras_insumos.forEach((i) => {
      if (i.gasto_id) {
        assert(cajaIds.has(i.gasto_id), `Insumo gasto_id ${i.gasto_id} not in caja_chica`);
        linkedCount++;
      }
    });
  ...
  test('No duplicate IDs in viajes', () => {
    ...
    const uniqueIds = new Set(ids);
    assert(ids.length === uniqueIds.size, `Duplicate IDs in viajes`);
  });

  test('No duplicate IDs in personal.trabajadores', () => {
    const ids = datos.personal.trabajadores.map((t) => t.id);
    const uniqueIds = new Set(ids);
    assert(ids.length === uniqueIds.size, `Duplicate IDs in trabajadores`);
  });
}
...
function auditTemplateUI() {
  ...
  test('PDF template has proper page structure', () => {
    ...
    assert(htmlContent.includes('class="pdf-page-header"'), 'Missing pdf-page-header class');
    assert(htmlContent.includes('class="tabla-pdf"'), 'Missing tabla-pdf class');
  });

  test('PDF template has proper CSS for overflow control', () => {
    assert(
      htmlContent.includes('word-break: break-word'),
      'Missing word-break: break-word in table CSS'
    );
    assert(
      htmlContent.includes('overflow-wrap: break-word'),
  ...
  test('PDF template prevents row breaks inside pages', () => {
    assert(htmlContent.includes('page-break-inside: avoid'), 'Missing page-break-inside: avoid');
    assert(htmlContent.includes('break-inside: avoid'), 'Missing break-inside: avoid');
  });

  test('PDF template repeats header on each page', () => {
    assert(
      htmlContent.includes('display: table-header-group'),
  ...
  test('PDF template has fixed page dimensions', () => {
    assert(
      htmlContent.includes('width: 210mm') || htmlContent.includes('width: 816px'),
      'Missing fixed page width'
    );
    assert(
      htmlContent.includes('min-height: 297mm') || htmlContent.includes('min-height: 1123px'),
  ...
  test('PDF template has overflow hidden', () => {
    assert(
      htmlContent.includes('overflow: hidden'),
      'Missing overflow: hidden on template container'
    );
  });

  test('PDF template has proper text escaping', () => {
    assert(htmlContent.includes('_esc('), 'Missing _esc function for text escaping');
  });
  ...
  test('PDF template has no fixed-width elements that could overflow', () => {
    ...
    assert(problematic.length === 0, `Found ${problematic.length} elements wider than page width`);
  });
  ...
  test('PDF template images have max-width constraints', () => {
    ...
    const templateMatch = htmlContent.match(
      /<div id="plantilla-reporte-impresion"[\s\S]*?<\/div>\s*<\/div>/
    );
    const templateContent = templateMatch ? templateMatch[0] : htmlContent;

    const imgTags = templateContent.match(/<img[^>]*>/g) || [];
    const unconstrained = imgTags.filter(
      (img) => !img.includes('max-width') && !img.includes('width:')
    );
    assert(
  ...
  test('PDF template signature images are constrained', () => {
    assert(
      htmlContent.includes('max-height: 58px') || htmlContent.includes('max-height:30px'),
...
function auditFormatAndPagination() {
  ...
  test('PDF generation uses proper page size constants', () => {
    assert(
      htmlContent.includes('PAGE_PX = { letter: 816, a4: 794, legal: 816 }'),
      'Missing PAGE_PX constants'
    );
    assert(
      htmlContent.includes("PAGE_MM = { letter: '216mm', a4: '210mm', legal: '216mm' }"),
  ...
  test('PDF generation uses html2pdf with proper options', () => {
    assert(
      htmlContent.includes("html2pdf().set(opts).from(contenedor).toPdf().get('pdf')"),
      'Missing html2pdf call'
    );
    assert(htmlContent.includes('margin: 0'), 'Missing margin: 0 in html2pdf options');
    assert(
      htmlContent.includes("jsPDF: { unit: 'mm', format: pageSize, orientation: 'portrait' }"),
  ...
  test('PDF generation injects CSS variables for cloned document', () => {
    assert(
      htmlContent.includes('onclone: function(clonedDoc)'),
      'Missing onclone function for CSS variable injection'
    );
    assert(htmlContent.includes('CSS_VARS'), 'Missing CSS_VARS definition');
  });

  test('PDF generation stamps page numbers', () => {
    assert(htmlContent.includes('Página ${i} de ${total}'), 'Missing page number stamping');
    assert(htmlContent.includes('pdf.setPage(i)'), 'Missing pdf.setPage() call');
    assert(
      htmlContent.includes('pdf.internal.getNumberOfPages()'),
  ...
  test('PDF generation handles page breaks properly', () => {
    assert(
      htmlContent.includes(
        "pagebreak: { mode: ['css', 'legacy'], avoid: ['tr', 'img', '.signature-wrapper'] }"
      ),
  ...
  test('PDF template has proper page break CSS', () => {
    assert(htmlContent.includes('page-break-after: always'), 'Missing page-break-after: always');
    assert(htmlContent.includes('page-break-inside: avoid'), 'Missing page-break-inside: avoid');
  });

  test('PDF generation uses proper document height', () => {
    assert(
      htmlContent.includes('const docHeight = Math.max(Math.ceil(contenedor.scrollHeight), 1123)'),
  ...
  test('PDF generation waits for layout recalculation', () => {
    assert(
      htmlContent.includes('requestAnimationFrame'),
      'Missing requestAnimationFrame for layout recalculation'
    );
  });

  test('PDF generation handles page size selection', () => {
    assert(htmlContent.includes('id="vp-page-size"'), 'Missing vp-page-size selector');
    assert(
      htmlContent.includes('letter') && htmlContent.includes('a4') && htmlContent.includes('legal'),
      'Missing page size options'
    );
  });

  test('CSV export has proper encoding (UTF-8 BOM)', () => {
    assert(htmlContent.includes("let csv = '\\uFEFF'"), 'Missing UTF-8 BOM in CSV export');
  });

  test('CSV export has proper pagination', () => {
    assert(htmlContent.includes('paginarCSV'), 'Missing paginarCSV function');
    assert(htmlContent.includes('maxRowsPerPage'), 'Missing maxRowsPerPage configuration');
  });

  test('CSV export includes signatures section', () => {
    assert(htmlContent.includes('generarFirmasCSV'), 'Missing generarFirmasCSV function');
    assert(htmlContent.includes('FIRMAS DE APROBACIÓN'), 'Missing FIRMAS DE APROBACIÓN header');
  });

  test('CSV export includes company information', () => {
    assert(
      htmlContent.includes('csv += `# ${empresa}\\n`;') || htmlContent.includes('# CONSTRURAMSA'),
      'Missing company header in CSV'
    );
    assert(htmlContent.includes('# Fecha de generacion'), 'Missing generation date in CSV');
  });

  test('PDF template has proper footer area', () => {
    assert(
      htmlContent.includes('Página') && htmlContent.includes('de ${total}'),
  ...
  test('PDF template has signature area', () => {
    assert(
      htmlContent.includes('pdf-firma-jefe-proyectos'),
      'Missing jefe de proyectos signature area'
    );
    assert(
      htmlContent.includes('pdf-firma-gerente-comercial'),
      'Missing gerente comercial signature area'
    );
    assert(htmlContent.includes('pdf-nombre-visto-bueno'), 'Missing visto bueno signature area');
  });

  test('PDF template has proper color scheme for printing', () => {
    assert(htmlContent.includes('--pdf-blanco'), 'Missing white background variable');
    assert(htmlContent.includes('--pdf-azul-oscuro'), 'Missing dark blue variable');
    assert(htmlContent.includes('--pdf-gris-oscuro'), 'Missing dark gray text variable');
  });

  test('PDF generation avoids breaking on specific elements', () => {
    assert(
      htmlContent.includes("avoid: ['tr', 'img', '.signature-wrapper']"),
...
function auditMobileSpecific() {
  ...
  test('Report UI has responsive breakpoints', () => {
    ...
    assert(htmlContent.includes('@media (max-width: 480px)'), 'Missing 480px breakpoint');
    assert(htmlContent.includes('@media (max-width: 390px)'), 'Missing 390px breakpoint');
    assert(htmlContent.includes('@media (max-width: 360px)'), 'Missing 360px breakpoint');
  });

  test('Report forms stack on mobile', () => {
    assert(
      htmlContent.includes('.form-grid-2 { display: grid; grid-template-columns: 1fr 1fr;'),
      'Missing form-grid-2 base style'
    );
    const hasMobileOverride =
      htmlContent.includes('grid-template-columns: 1fr') && htmlContent.includes('.form-grid-2');
    assert(hasMobileOverride, 'Missing mobile override for form-grid-2');
  });

  test('Report buttons are full-width on mobile', () => {
    assert(
      htmlContent.includes('.btn-row { flex-direction: column; }'),
      'Missing btn-row column direction for mobile'
    );
  });

  test('Preview modal is responsive', () => {
    assert(htmlContent.includes('#modal-vista-previa'), 'Missing vista previa modal');
    assert(
      htmlContent.includes('.vp-editor') && htmlContent.includes('.vp-preview'),
      'Missing editor/preview split in modal'
    );
    assert(
      htmlContent.includes('max-height: 38dvh') || htmlContent.includes('max-height: 40dvh'),
  ...
  test('Toast notifications are responsive', () => {
    assert(
      htmlContent.includes('.toast-container { left: 10px !important; right: 10px !important;'),
  ...
  test('Tables have horizontal scroll on mobile', () => {
    assert(
      htmlContent.includes('.overflow-x-auto'),
  ...
  test('PDF generation uses proper pixel-to-mm conversion', () => {
    assert(
      htmlContent.includes('816') && htmlContent.includes('794'),
...
function main() {
  ...
  logInfo(
    `Report Types: diario, semanal, mensual, asistencia, viajes, mantenimiento, categoria, nomina, ejecutivo\n`
  );
