...
function analyzeLetterhead() {
  ...
  const redundantElements = {
    'Company name':
      htmlContent.includes('pdf-nombre-empresa') && htmlContent.includes('.brand-name'),
    'Project name':
      htmlContent.includes('pdf-nombre-proyecto') && htmlContent.includes('Proyecto:'),
    'Contact info':
      htmlContent.includes('pdf-tel-email-empresa') && htmlContent.includes('telefono'),
  ...
  logInfo('\n--- Visual Structure ---');
  const hasTable = htmlContent.includes(
  ...
  // Check borders
  const hasBorders =
    htmlContent.includes('border-left: 1px solid #E5E7EB') &&
...
function analyzePageHeader() {
  ...
  logInfo('\n--- Page Header CSS ---');
  const hasGradient = htmlContent.includes(
    'background: linear-gradient(135deg, var(--pdf-azul-oscuro) 0%, var(--pdf-azul-claro) 100%)'
  );
  if (hasGradient) {
...
function analyzeAlignmentSpacing() {
  ...
  // Check column widths
  const hasLeftColumn =
    htmlContent.includes('width: 22%') && htmlContent.includes('border-right: 1px solid #E5E7EB');
  const hasCenterColumn =
    htmlContent.includes('width: 56%') && htmlContent.includes('text-align: center');
  const hasRightColumn =
    htmlContent.includes('width: 22%') && htmlContent.includes('border-left: 1px solid #E5E7EB');
  ...
  const hasVerticalAlign =
    htmlContent.includes('vertical-align: top') && htmlContent.includes('vertical-align: middle');
  if (hasVerticalAlign) {
...
function analyzeVisualCoherence() {
  ...
  const letterheadUsesBlue =
    htmlContent.includes('#004B93') || htmlContent.includes('var(--pdf-azul-oscuro)');
  const letterheadUsesWhite =
    htmlContent.includes('#ffffff') || htmlContent.includes('var(--pdf-blanco)');
  ...
  const pageHeaderUsesGradient = htmlContent.includes(
    'linear-gradient(135deg, var(--pdf-azul-oscuro) 0%, var(--pdf-azul-claro) 100%)'
  );
  ...
  // Check font sizes
  const hasFontSizes =
    htmlContent.includes('font-size: 22px') && // Company name
  ...
  const hasLetterSpacing =
    htmlContent.includes('letter-spacing: 6px') || htmlContent.includes('letter-spacing: 1.5px');
  if (hasLetterSpacing) {
...
function analyzeRedundancy() {
  ...
  if (hasLetterhead && hasPageHeader) {
    ...
    addRecommendation(
      'CRITICAL',
      'Remove static letterhead and rely on repeating page header, OR simplify letterhead to avoid redundancy'
    );
  }
  ...
  const letterheadWhite =
    htmlContent.includes('background: #ffffff') ||
    htmlContent.includes('background: var(--pdf-blanco)');
