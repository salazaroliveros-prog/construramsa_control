...
function auditDataIntegrity(db) {
  ...
  if (!db.proyectos || db.proyectos.length === 0) {
    ...
    const duplicateNames = projectNames.filter(
      (item, index) => projectNames.indexOf(item) !== index
    );
    if (duplicateNames.length > 0) {
      issues.push(`Duplicate project names: ${duplicateNames.join(', ')}`);
      auditResults.dataIntegrity.errors.push(
        `Duplicate project names: ${duplicateNames.join(', ')}`
      );
    } else {
  ...
  if (db.proyectos_data) {
    Object.keys(db.proyectos_data).forEach((pid) => {
      ...
      // Check caja_chica
      if (data.caja_chica) {
        const invalidMontos = data.caja_chica.filter(
          (m) => typeof m.monto !== 'number' || isNaN(m.monto)
        );
      ...
      if (data.viajes_camiones && data.viajes_camiones.viajes) {
        const invalidTotals = data.viajes_camiones.viajes.filter(
          (v) => typeof v.total !== 'number' || isNaN(v.total)
        );
...
function auditAccessibility(indexHtml) {
  ...
  if (labelElements < inputElements * 0.5) {
    ...
    auditResults.accessibility.errors.push(
      `Insufficient labels: ${labelElements}/${inputElements}`
    );
  } else {
