...
function cargarBaseDatos() {
  try {
    if (!fs.existsSync(DB_FILE)) {
      logError(
        'No existe el archivo de base de datos. Primero debes usar la aplicación para ingresar datos reales.'
      );
      return null;
    }
...
function validarCajaChica(db) {
  ...
  if (totalMovimientos === 0) {
    logWarning(
      'No hay movimientos de caja chica registrados. Ingresa movimientos desde la aplicación.'
    );
  } else {
  ...
  if (db.proyectos_data) {
    Object.keys(db.proyectos_data).forEach((pid) => {
      ...
      if (datosProyecto.maquinaria_flota && datosProyecto.maquinaria_flota.registros) {
        logInfo(
          `Proyecto "${nombreProyecto}": ${datosProyecto.maquinaria_flota.registros.length} registros`
        );
  ...
  if (db.proyectos_data) {
    Object.keys(db.proyectos_data).forEach((pid) => {
      ...
      if (datosProyecto.personal && datosProyecto.personal.trabajadores) {
        logInfo(
          `Proyecto "${nombreProyecto}": ${datosProyecto.personal.trabajadores.length} trabajadores`
        );
...
function validarAdquisiciones(db) {
  ...
  if (db.adquisiciones) {
    ...
    if (db.adquisiciones.cotizaciones_compras) {
      logInfo(
        `Encontradas ${db.adquisiciones.cotizaciones_compras.length} cotizaciones en datos generales`
      );
    ...
    Object.keys(db.proyectos_data).forEach((pid) => {
      ...
      if (datosProyecto.adquisiciones) {
        if (datosProyecto.adquisiciones.proveedores) {
          logInfo(
            `Proyecto "${nombreProyecto}": ${datosProyecto.adquisiciones.proveedores.length} proveedores`
          );
        ...
        if (datosProyecto.adquisiciones.cotizaciones_compras) {
          logInfo(
            `Proyecto "${nombreProyecto}": ${datosProyecto.adquisiciones.cotizaciones_compras.length} cotizaciones`
          );
  ...
  if (db.proyectos_data) {
    Object.keys(db.proyectos_data).forEach((pid) => {
      ...
      if (datosProyecto.viajes_camiones && datosProyecto.viajes_camiones.viajes) {
        logInfo(
          `Proyecto "${nombreProyecto}": ${datosProyecto.viajes_camiones.viajes.length} viajes`
        );
...
function validarMantenimiento(db) {
    ...
    Object.keys(db.proyectos_data).forEach((pid) => {
      ...
      if (datosProyecto.mantenimiento) {
        if (datosProyecto.mantenimiento.ordenes) {
          logInfo(
            `Proyecto "${nombreProyecto}": ${datosProyecto.mantenimiento.ordenes.length} órdenes`
          );
        ...
        if (datosProyecto.mantenimiento.compras_insumos) {
          logInfo(
            `Proyecto "${nombreProyecto}": ${datosProyecto.mantenimiento.compras_insumos.length} insumos`
          );
  ...
  if (totalOrdenes === 0) {
    logWarning(
      'No hay órdenes de mantenimiento registradas. Registra mantenimientos desde la aplicación.'
    );
  } else {
  ...
  if (totalInsumos === 0) {
    logWarning(
      'No hay compras de insumos registradas. Registra compras de insumos desde la aplicación.'
    );
  } else {
...
function validarConsistenciaFinanciera(db) {
  ...
  const validarMontos = (array, contexto) => {
    ...
    array.forEach((item, index) => {
      ...
      if (
        item.presupuesto !== undefined &&
        (typeof item.presupuesto !== 'number' || isNaN(item.presupuesto))
      ) {
...
function validarFechas(db) {
  ...
  if (db.proyectos_data) {
    Object.keys(db.proyectos_data).forEach((pid) => {
      ...
      if (datos.caja_chica) {
        datos.caja_chica.forEach((mov, i) =>
          validarFecha(mov.fecha, `Movimiento caja ${nombre} ${i + 1}`)
        );
      }
      if (datos.maquinaria_flota?.registros) {
        datos.maquinaria_flota.registros.forEach((reg, i) =>
          validarFecha(reg.fecha, `Registro maquinaria ${nombre} ${i + 1}`)
        );
      }
      if (datos.personal?.asistencia) {
        datos.personal.asistencia.forEach((asi, i) =>
          validarFecha(asi.fecha, `Asistencia ${nombre} ${i + 1}`)
        );
      }
      if (datos.viajes_camiones?.viajes) {
        datos.viajes_camiones.viajes.forEach((viaje, i) =>
          validarFecha(viaje.fecha, `Viaje ${nombre} ${i + 1}`)
        );
      }
      if (datos.mantenimiento?.ordenes) {
        datos.mantenimiento.ordenes.forEach((orden, i) =>
          validarFecha(orden.fecha, `Orden mantenimiento ${nombre} ${i + 1}`)
        );
      }
    });
  }
