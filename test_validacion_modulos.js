...
function registro(modulo, accion, ok, detalle = '') {
  ...
  if (nivel === 'pass') {
    suite.modulos[modulo].pass++;
    suite.totales.pass++;
  } else if (nivel === 'fail') {
    suite.modulos[modulo].fail++;
    suite.totales.fail++;
  } else {
    suite.totales.warn++;
  }
...
async function getDB(page) {
  return page.evaluate(() => {
    try {
      return JSON.parse(localStorage.getItem('construramsa_db') || '{}');
    } catch (_) {
      return {};
    }
  });
...
async function testResumen(page) {
  ...
  try {
    ...
    const panel = page.locator(
      '.resumen-alertas, .alert-item, .sin-proyecto, .estado-vacio, .empty-state, #resumen'
    );
...
async function testAdquisiciones(page) {
  ...
  try {
    ...
    const btn = page
      .locator('button:has-text("Agregar"), button:has-text("Nuevo"), button:has-text("Guardar")')
      .first();
...
async function testViajes(page) {
  ...
  try {
    ...
    registro(
      M,
      'Selector #cam-propiedad (propio/alquilado) presente',
      (await selPropiedad.count()) > 0
    );
...
async function testMantenimiento(page) {
  ...
  try {
    ...
    registro(
      M,
      'Formulario #form-orden (órdenes/inspección) presente',
      (await formOrden.count()) > 0
    );
    registro(
      M,
      'Formulario #form-insumo (compras insumos) presente',
      (await formInsumo.count()) > 0
    );
...
async function testReportes(page) {
  ...
  try {
    ...
    const btnDescargar = page
      .locator('button[onclick*="abrirVistaPrevia"], button[onclick*="generarReporteLocal"]')
      .first();
    registro(
      M,
      'Botón descargar reporte (abrirVistaPrevia/generarReporteLocal) presente',
      (await btnDescargar.count()) > 0
    );
    ...
    // Zona de preview
    const preview = page
      .locator('#plantilla-reporte-impresion, #preview-reporte, [id*="preview"]')
      .first();
    ...
    try {
      ...
      if (await btnPreview.isVisible({ timeout: 2000 })) {
        ...
        registro(
          M,
          'Botón abrirVistaPrevia',
          'warn',
          'No visible — puede requerir fecha seleccionada'
        );
      }
    } catch (e2) {
...
async function testConfiguracion(page) {
  ...
  try {
    ...
    // Campos de empresa
    const campoEmpresa = page
      .locator('input[placeholder*="empresa"], input[id*="empresa"]')
      .first();
    const campoEslogan = page
      .locator('input[placeholder*="slogan"], input[placeholder*="Eslogan"], input[id*="eslogan"]')
      .first();
    ...
    registro(
      M,
      'Input #import-json (importar base de datos) presente',
      (await inputImportar.count()) > 0
    );
    ...
    // Sección de nube
    const secNube = page
      .locator('[id*="nube"], button:has-text("Nube"), button:has-text("Cloud")')
      .first();
...
async function testCrossModulo(page) {
  ...
  try {
    ...
    const label = await tabActivo
      .first()
      .textContent()
      .catch(() => '');
    registro(
      M,
      `Deep-link ?module=viajes activa tab correcto ("${label.trim()}")`,
...
async function testFlujoIntegracion(page) {
  ...
  try {
    ...
    registro(
      M,
      'Caja Chica carga con formulario activo',
      await formCaja.isVisible({ timeout: 3000 }).catch(() => false)
    );
    ...
    registro(
      M,
      'Maquinaria carga con formulario activo',
      await formMaq.isVisible({ timeout: 3000 }).catch(() => false)
    );
    ...
    registro(
      M,
      'Viajes carga con formulario activo',
      await formViaje.isVisible({ timeout: 3000 }).catch(() => false)
    );
    ...
    registro(
      M,
      'Reportes carga con selector de tipo activo',
      await selReporte.isVisible({ timeout: 3000 }).catch(() => false)
    );
  } catch (e) {
