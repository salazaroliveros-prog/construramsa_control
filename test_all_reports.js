...
function numToInt(v) {
  const n = parseFloat(v);
  return (Number.isFinite(n) ? Math.round(n) : 0).toString();
}
...
function testReportGeneration(tipo, db) {
  ...
  if (!db.proyectos || db.proyectos.length === 0) {
    logWarning(
      'No projects found in database; skipping report generation for clean-state validation.'
    );
    return {
      passed: true,
      failed: false,
  ...
  switch (tipo) {
    ...
    case 'mensual':
      ...
      const movs = (datos.caja_chica || []).filter(
        (g) => enRango(g.fecha) && !idsExternos.has(g.id)
      );
