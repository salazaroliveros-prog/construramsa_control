...
const modulos = [
  {
    name: 'Caja Chica',
    tab: '#tab-caja-chica',
    container: '#caja-chica',
    title: '#caja-chica-title',
    extra: ['#saldo-caja-chica', '#form-caja-chica', '#tabla-caja-chica'],
  },
  {
    name: 'Maquinaria',
    tab: '#tab-maquinaria',
    container: '#maquinaria',
    title: '#maquinaria-title',
    extra: ['#form-maquinaria', '#tabla-maquinaria'],
  },
  {
    name: 'Personal',
    tab: '#tab-personal',
    ...
    extra: ['#form-trabajador', '#tabla-asistencia'],
  },
  {
    name: 'Viajes Camiones',
    tab: '#tab-viajes',
    container: '#viajes',
    title: '#viajes-title',
    extra: ['#form-camion', '#tabla-camiones', '#form-viaje', '#tabla-viajes'],
  },
  {
    name: 'Mantenimiento',
    ...
    extra: ['#form-maqcat', '#tabla-ordenes', '#form-insumo', '#tabla-insumos'],
  },
  {
    name: 'Adquisiciones',
    tab: '#tab-adquisiciones',
    container: '#adquisiciones',
    title: '#adquisiciones-title',
    extra: ['#form-proveedor', '#tabla-proveedores', '#form-cotizacion', '#tabla-cotizaciones'],
  },
  {
    name: 'Reportes',
    tab: '#tab-reportes',
    ...
    extra: ['#reporte-tipo', '#export-pdf', '#export-csv', '#reporte-fecha'],
  },
  {
    name: 'Configuracion',
    ...
    extra: ['#cfg-nombre-empresa', '#btn-instalar-app2'],
  },
  {
    name: 'Dashboard Costos',
    tab: '#tab-resumen',
    container: '#resumen',
    title: '#resumen .modulo-titulo',
    extra: ['#kpi-grid', '#grafico-gastos', '#dashboard-categoria'],
  },
];
  ...
  for (const m of modulos) {
    try {
      ...
      for (const sel of m.extra) {
        ...
        if (!ok) {
          extraDetails.push(sel);
          extrasOk = false;
        }
      }
      addResult(
        'Modulos',
        m.name + ' - selectores extra',
        extrasOk,
        extrasOk ? '' : extraDetails.join(', ')
      );

      const safe = m.name.replace(/\s+/g, '_');
      await page.screenshot({
        path: path.join(OUT_DIR, 'modulo_' + safe + '.png'),
        fullPage: true,
      });
    } catch (e) {
  ...
  try {
    ...
    if (catSelect) {
      const catValue = await page.evaluate(() => {
        const opts = Array.from(document.querySelectorAll('#dashboard-categoria option')).map(
          (o) => o.value
        );
        return opts.find((v) => /material/i.test(v)) || (opts.length > 1 ? opts[1] : null);
      });
      const labelsAntes = await page.evaluate((sel) => {
        return Array.from(document.querySelectorAll(sel + ' .css-chart-bar')).map((e) =>
          e.getAttribute('data-category')
        );
      }, chartBase);
      addResult(
        'Dashboard',
        'Barras/labels visibles antes filtro',
        labelsAntes.length > 0,
        'count=' + labelsAntes.length + ' cats=' + labelsAntes.join('|')
      );
      ...
      addResult(
        'Dashboard',
        'Grafico no vacio tras filtro (regresion BUG-1)',
        noVacio,
        'labels count=' + labelsDespues.length
      );

      const bgColors = await page.evaluate((sel) => {
        const barras = Array.from(document.querySelectorAll(sel + ' .css-chart-bar'));
        return barras.map(
          (b) => getComputedStyle(b).backgroundImage || getComputedStyle(b).backgroundColor
        );
      }, chartBase);
      if (bgColors.length > 1) {
        ...
        addResult(
          'Dashboard',
          'Colores diferenciados entre categorias (regresion BUG-2)',
          unique.size > 1,
        ...
        addResult(
          ...
          'Colores diferenciados entre categorias (regresion BUG-2)',
          false,
          'insufficient bars: ' + bgColors.length
        );
      }
      await page.screenshot({
        path: path.join(OUT_DIR, 'dashboard_filtro_categoria.png'),
        fullPage: true,
      });
    } else {
      addResult(
        'Dashboard',
        'Filtro categoria presente',
        false,
        '#dashboard-categoria no encontrado'
      );
    }
  } catch (e) {
...
async function testResponsive(page) {
  ...
  for (const vp of viewports) {
    ...
    addResult(
      'Responsive',
      vp.name + ' - tabs visibles',
      tabs > 0,
      'view=' + vp.width + 'x' + vp.height
    );
    await page.screenshot({
      path: path.join(OUT_DIR, 'responsive_' + vp.name + '.png'),
      fullPage: true,
    });
  }
}
...
async function testPDFExport(page) {
  ...
  try {
    ...
    if (pdfChk && csvChk && tipoSel) {
      ...
      addResult(
        'Reportes',
        'Controles PDF/CSV presentes',
        true,
        '#export-pdf + #export-csv + #reporte-tipo'
      );
      addResult(
        'Reportes',
        'PDF ejecutivo generado',
        previewOk === 'ok',
        'abrirVistaPrevia(local) -> ' + previewOk
      );
    } else {
...
async function generateReport() {
  let html =
    '<!DOCTYPE html><html><head><meta charset="utf-8"><title>E2E Validation Report</title>';
  ...
  html +=
    'h1{color:#2c3e50}.summary{display:flex;gap:20px;margin:20px 0}.card{flex:1;padding:15px;border-radius:8px;text-align:center}';
  ...
  html +=
    '.section{margin-top:30px}img{max-width:300px;margin:10px;display:block;border:1px solid #ddd;border-radius:4px}</style></head><body>';
  ...
  for (const sec of sections) {
    html +=
      ...
      sec +
      '</h2><table><tr><th>Test</th><th>Status</th><th>Detalles</th></tr>';
