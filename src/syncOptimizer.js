...
const SyncOptimizer = (() => {
  const safeStorage = (() => {
    try {
      ...
      // localStorage no disponible en este contexto
    }
    return null;
  })();
  ...
  function _saveMetrics() {
    ...
    try {
      ...
      // Error al guardar métricas, continuar sin interrumpir
    }
  }
  ...
  function _saveRetryLog() {
    ...
    try {
      ...
      // Error al guardar log de reintentos, continuar sin interrumpir
    }
  }
  ...
  function _recordRetry(endpoint, attemptNumber, errorMessage, nextBackoffMs) {
    ...
    _saveRetryLog();
    console.warn(
      `[RETRY ${attemptNumber}/${CONFIG.MAX_RETRIES}] ${endpoint}: ${errorMessage} → retry in ${nextBackoffMs}ms`
    );
  }
