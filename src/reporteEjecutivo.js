...
(function (globalScope) {
  ...
  function fmtQ(n) {
    if (
      ...
      globalScope.CR_Export &&
      globalScope.CR_Export.formatearMonto
    ) {
  ...
  function calcularKPIs(db, opts) {
    ...
    var db2 = db || {};
    if (
      globalScope.CR_ReportData &&
      typeof globalScope.CR_ReportData.resolve === 'function' &&
      db2.proyectos_data &&
      db2.configuracion
    ) {
    ...
    if (
      globalScope.CR_ReportData &&
      typeof globalScope.CR_ReportData.normalizeProjectData === 'function'
    ) {
  ...
  function _enriquecerKPIsParaReporte(kpisEngine, db2, inicio, fin) {
    var porCategoria =
      kpisEngine && kpisEngine.gastosPorCategoria ? kpisEngine.gastosPorCategoria : {};
    var porDia = {};
    ...
    var viajesCount = 0;
    (Array.isArray((db2.viajes_camiones || {}).viajes) ? db2.viajes_camiones.viajes : []).forEach(
      function (v) {
        if (v && enRango(fechaMov(v), inicio, fin)) viajesCount++;
      }
    );
    ...
    (Array.isArray((db2.personal || {}).asistencia) ? db2.personal.asistencia : []).forEach(
      function (dia) {
  ...
  function _calcularKPIsLocal(db2, inicio, fin) {
    ...
    (Array.isArray((db2.viajes_camiones || {}).viajes) ? db2.viajes_camiones.viajes : []).forEach(
      function (v) {
        if (v && enRango(fechaMov(v), inicio, fin)) kpis.viajes++;
      }
    );
  ...
  function _alertas(kpis, db2) {
    ...
    if (presupuesto > 0 && kpis.totalGastos > 0) {
      ...
      if (ejecucion >= 90)
        kpis.alertas.push({
          nivel: 'danger',
          texto: 'Ejecución presupuestaria al ' + fmtPct(ejecucion, 0) + ' — riesgo de sobregiro.',
        });
      else if (ejecucion >= 70)
        kpis.alertas.push({
          nivel: 'warning',
          texto: 'Ejecución presupuestaria al ' + fmtPct(ejecucion, 0) + ' — monitorear remanente.',
        });
    }
    ...
    if (cats.length && kpis.totalGastos > 0) {
      ...
      cats.forEach(function (c) {
        if (kpis.porCategoria[c] > kpis.porCategoria[mayor]) mayor = c;
      });
      ...
      if (share >= 50)
        kpis.alertas.push({
          nivel: 'warning',
          texto:
            'La categoría «' + esc(mayor) + '» concentra el ' + fmtPct(share, 0) + ' del gasto.',
        });
    }
    if (kpis.totalGastos > 0 && kpis.saldo < 0) {
      kpis.alertas.push({
        nivel: 'danger',
        texto:
          'Egresos superan a ingresos del período (déficit ' + fmtQ(Math.abs(kpis.saldo)) + ').',
      });
    }
    ...
    if (kpis.alertas.length === 0)
      kpis.alertas.push({ nivel: 'info', texto: 'Operación estable: sin hallazgos relevantes.' });
    return kpis;
  }
  ...
  function svgBarrasCategorias(porCategoria, maxItems) {
    var items = Object.keys(porCategoria || {})
      .map(function (k) {
        return { name: k, value: num(porCategoria[k]) };
      })
      .sort(function (a, b) {
    ...
    if (total <= 0) {
      return (
        ...
        COLORES.gris +
        ';font-weight:600;">Sin datos para graficar</div>' +
        '<div style="font-size:11px;color:' +
        COLORES.gris +
        ';margin-top:4px;">Registra movimientos de gastos para visualizar la distribución</div>' +
        '</div>'
      );
    }
  ...
  function svgTendencia(porDia) {
    ...
    if (dias.length === 0) {
      return (
        ...
        '<div style="font-size:13px;color:' +
        COLORES.gris +
        ';font-weight:600;">Sin tendencia disponible</div>' +
        '<div style="font-size:11px;color:' +
        COLORES.gris +
        ';margin-top:4px;">Registra movimientos diarios para visualizar la tendencia de gastos</div>' +
        '</div>'
      );
    }
    ...
    var innerW = w - padL - padR,
      innerH = h - padT - padB;
    var vals = dias.map(function (d) {
      return num((porDia[d] || {}).gastos);
    });
    var maxV = Math.max.apply(null, vals.concat([1])) * 1.1;

    function px(i, v) {
      return {
        x: padL + (i / (dias.length - 1 || 1)) * innerW,
    ...
    return (
      ...
      '" width="100%" height="' +
      h +
      '" role="img" aria-label="Tendencia de gastos diarios">' +
      grid +
      '<polygon points="' +
      polyArea +
      '" fill="' +
  ...
  function generarHTML(db, portada, opts) {
    ...
    var kpiCards =
      ...
      cardKPI(
        ...
        kpis.saldo >= 0 ? COLORES.acento : COLORES.alerta
      ) +
    ...
    var kpiOp =
      cardKPI(
        'Combustible',
        fmtQ(kpis.maquinaria.costoCombustible),
        kpis.maquinaria.galones.toFixed(1) + ' galones'
      ) +
      cardKPI(
        'Mantenimiento',
        fmtQ(kpis.maquinaria.costoMantenimiento),
        kpis.maquinaria.registros + ' registros'
      ) +
      cardKPI('Viajes', String(kpis.viajes), 'viajes de camiones') +
      cardKPI(
        'Asistencia',
        String(kpis.asistencia.registros),
        kpis.asistencia.ausencias + ' ausencias'
      );

    var adquisiciones = (db.adquisiciones || {}).cotizaciones_compras || [];
    var comprasPeriodo = adquisiciones.filter(function (c) {
      return enRango(fechaMov(c), opts.inicio, opts.fin) && c.estado === 'aprobada';
    });
    var totalCompras = comprasPeriodo.reduce(function (s, c) {
      return s + num(c.total);
    }, 0);
    var proveedoresUnicos = {};
    comprasPeriodo.forEach(function (c) {
      proveedoresUnicos[c.proveedor_id] = true;
    });
    ...
    var proveedorNombre = {};
    proveedoresDb.forEach(function (p) {
      proveedorNombre[p.id] = p.nombre || 'Sin nombre';
    });
    var resumenProveedores = {};
    comprasPeriodo.forEach(function (c) {
      var pid = c.proveedor_id || 'sin_proveedor';
      if (!resumenProveedores[pid])
        resumenProveedores[pid] = {
          nombre: proveedorNombre[pid] || 'Desconocido',
          total: 0,
          cantidad: 0,
        };
    ...
    var filasProv = Object.keys(resumenProveedores)
      .sort(function (a, b) {
        return resumenProveedores[b].total - resumenProveedores[a].total;
      })
      .map(function (pid, i) {
        ...
        return (
          '<tr>' +
          '<td style="padding:7px 10px;border-bottom:1px solid ' +
          COLORES.lineas +
          ';color:' +
          COLORES.gris +
          ';font-size:12px;">' +
    ...
    var tablaProvHtml = filasProv.length
      ? '<div style="border:1px solid ' +
        ...
        COLORES.fondo +
        ';">' +
        '<th style="padding:8px 10px;text-align:left;font-size:11px;color:' +
        COLORES.gris +
        ';">#</th>' +
        '<th style="padding:8px 10px;text-align:left;font-size:11px;color:' +
        COLORES.gris +
        ';">Proveedor</th>' +
        '<th style="padding:8px 10px;text-align:right;font-size:11px;color:' +
        COLORES.gris +
        ';">Compras</th>' +
        '<th style="padding:8px 10px;text-align:right;font-size:11px;color:' +
        COLORES.gris +
    ...
    var alertasHtml = kpis.alertas
      .map(function (a) {
        ...
        return (
          '<div style="display:flex;gap:10px;align-items:flex-start;padding:10px 12px;border:1px solid ' +
          COLORES.lineas +
    ...
    var cats = Object.keys(kpis.porCategoria)
      .sort(function (a, b) {
        return kpis.porCategoria[b] - kpis.porCategoria[a];
      })
      .slice(0, 8);
    var tablaRows = cats.length
      ? cats
          ...
          .join('')
      : '<tr><td colspan="4" style="padding:12px;color:' +
        COLORES.gris +
        ';font-size:13px;text-align:center;">Sin movimientos de gasto en el período.</td></tr>';
    ...
    return (
      ...
      (portada.logo
        ? '<img src="' + esc(portada.logo) + '" style="height:64px;object-fit:contain;" alt="">'
        : '<div style="width:64px;height:64px;border-radius:12px;background:' +
          COLORES.primario +
          ';color:#fff;display:flex;align-items:center;justify-content:center;font-size:26px;font-weight:800;">' +
          (empresa.charAt(0) || 'C') +
          '</div>') +
      '<div><div style="font-size:11px;letter-spacing:2px;color:' +
      COLORES.gris +
      ';text-transform:uppercase;">Reporte Ejecutivo</div>' +
      ...
      kpiCards +
      '</div>' +
      '<div class="ej-grid" style="margin-top:10px;">' +
      kpiOp +
      '</div>' +
      '<div class="ej-grid" style="margin-top:10px;">' +
      kpiAdq +
      '</div>' +
      // ---- ANÁLISIS ----
      '<div class="ej-page-break">' +
      tituloSeccion('Distribución de Gastos por Categoría') +
      '</div>' +
      '<div style="display:flex;gap:18px;align-items:flex-start;">' +
      '<div style="flex:0 0 56%;">' +
      svgBarrasCategorias(kpis.porCategoria) +
      '</div>' +
      '<div style="flex:1;min-width:0;">' +
      tablaCategorias(tablaRows) +
      '</div>' +
      '</div>' +
      '<div class="ej-page-break" style="margin-top:20px;">' +
      tituloSeccion('Tendencia de Gastos Diarios') +
      '</div>' +
      svgTendencia(kpis.porDia) +
      '<div class="ej-page-break" style="margin-top:22px;">' +
      tituloSeccion('Alertas Gerenciales') +
      '</div>' +
      alertasHtml +
      '<div class="ej-page-break" style="margin-top:22px;">' +
      tituloSeccion('Resumen de Adquisiciones') +
      '</div>' +
      '<div class="info-box"><strong>Compras del período</strong> ' +
      proveedoresActivos +
      ' proveedor(es) activo(s) · Total: ' +
      fmtQ(totalCompras) +
      '</div>' +
      tablaProvHtml +
      ...
      COLORES.lineas +
      ';padding-top:8px;width:240px;">' +
      '<div style="font-size:13px;font-weight:600;color:' +
      COLORES.tinta +
      ';">' +
      esc(portada.responsable || 'Gerente de Proyecto') +
      '</div>' +
      '<div style="font-size:11px;color:' +
      COLORES.gris +
      ';">Firma y sello</div></div></div>' +
      '<div style="margin-top:16px;padding-top:12px;border-top:1px solid ' +
      COLORES.lineas +
      ';font-size:10px;color:' +
      COLORES.gris +
      ';text-align:center;">' +
      'Informe generado por CONSTRURAMSA · ' +
      empresa +
      ' · Documento confidencial de uso gerencial</div>' +
      ...
      esc(rangoTxt) +
      '</div>' +
      '<div class="ej-footer-right">Generado: ' +
      esc(fechaEmision) +
      ' ' +
      esc(horaEmision) +
      ...
      '</div>' +
      '</div>'
    ); // ← cierre de div.ej-cover
  }

  function cardKPI(titulo, valor, sub, color) {
    var c = color || COLORES.primarioOsc;
    return (
      '<div style="flex:1;min-width:130px;background:' +
      COLORES.blanco +
      ';border:1px solid ' +
      COLORES.lineas +
      ';border-radius:10px;padding:12px 14px;border-top:3px solid ' +
      c +
      ';">' +
      '<div style="font-size:11px;color:' +
      COLORES.gris +
  ...
  function tituloSeccion(txt) {
    return (
      '<h2 style="font-size:16px;font-weight:800;color:' +
      COLORES.primarioOsc +
      ';margin:0 0 12px;padding-bottom:6px;border-bottom:2px solid ' +
      COLORES.primario +
      ';">' +
      txt +
      '</h2>'
