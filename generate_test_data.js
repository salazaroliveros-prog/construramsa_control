...
function generateComprehensiveTestData(db) {
  ...
  const workers = [
    ...
    {
      id: generateId('t'),
      nombre: 'Pedro Sánchez Rodríguez',
      ...
      return {
        ...
        calculos:
          estado === 'asistio'
            ? {
                total_diario:
                  8 * trabajador.pago_hora_normal + horasExtras * trabajador.pago_hora_extra,
  ...
  const suppliers = [
    {
      id: generateId('prov'),
      nombre: 'Constructora Los Ángeles S.A.',
      telefono: '5555-1234',
    ...
    {
      id: generateId('prov'),
      nombre: 'Distribuidora de Cemento Nacional',
      telefono: '5555-5678',
      email: 'info@cementonacional.com',
      direccion: 'Carretera a El Salvador',
      contacto: 'Marta López',
    },
    {
      id: generateId('prov'),
      nombre: 'Hierros y Aceros del Norte',
      telefono: '5555-9012',
      email: 'ventas@hierrosnorte.com',
      direccion: 'Zona 18, Guatemala',
      contacto: 'Roberto Díaz',
    },
    {
      id: generateId('prov'),
      nombre: 'Materiales de Construcción Bella Vista',
      telefono: '5555-3456',
  ...
  dates.slice(0, 18).forEach((fecha, idx) => {
    ...
    linkedExpenses.set(gastoId, {
      module: 'viajes',
      description: `Viaje ${camion.nombre}`,
      amount: viaje.total,
    });
  ...
  dates.slice(0, 12).forEach((fecha, idx) => {
    ...
    linkedExpenses.set(gastoId, {
      ...
      description: `Mantenimiento ${maquina.nombre}`,
      amount: orden.costo,
    });
  ...
  const vehiculosFlota = [
    {
      ...
      marca_modelo: 'CAT 320D',
    },
    {
      id: generateId('veh'),
      nombre: 'Excavadora Hidráulica',
      tipo: 'excavadora',
      marca_modelo: 'Komatsu PC200',
    },
    {
      id: generateId('veh'),
      nombre: 'Camión de Volteo',
      tipo: 'camion',
      marca_modelo: 'Volvo FM12',
    },
  ];
  ...
  console.log(
    `   🏗️ Maquinaria Flota: ${vehiculosFlota.length} vehicles, ${datos.maquinaria_flota.registros.length} records`
  );
  console.log(
    `   👷 Personal: ${workers.length} workers, ${attendanceDates.length} attendance days`
  );
  console.log(
    `   🏭 Adquisiciones: ${suppliers.length} suppliers, ${datos.adquisiciones.cotizaciones_compras.length} quotations`
  );
  console.log(
    `   🚛 Viajes: ${rutas.length} routes, ${camiones.length} trucks, ${datos.viajes_camiones.viajes.length} trips`
  );
  console.log(
    `   🔧 Mantenimiento: ${maquinariaMant.length} machines, ${datos.mantenimiento.ordenes.length} orders, ${datos.mantenimiento.compras_insumos.length} supplies`
  );
